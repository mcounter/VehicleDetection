...
recreateStorage = True
# Set this parameter True to train on full data set.
# When optimal parameters found, use this option
full_train = True

train_data = DeepDataEngine('train')
valid_data = DeepDataEngine('valid')

if recreateStorage:
    # Initialize image engine. Use default setup or saved in file
    imgEng = ImageEngine(load_setup = True)

    train_plan, test_plan = DeepDataEngine.createGenerationPlan('./img_data/vehicles', './img_data/non_vehicles', testSplit = 0.2)

    train_data.createStorage(train_plan, imgEng = imgEng, savePreProcessedFeatures = True, override = True)
    valid_data.createStorage(test_plan, imgEng = imgEng, savePreProcessedFeatures = True, override = True)
else:
